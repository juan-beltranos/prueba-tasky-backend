...
export class UserService {
    ...
    async createUser(data: { name: string; phone: string; profilePictureFile: Express.Multer.File }) {
    ...
    async addMessageToUser(phone: string, messageData: { message: string; timestamp: admin.firestore.Timestamp }) {
        const usersCollection = this.firestore.collection('users');
        const snapshot = await usersCollection.where('phone', '==', phone).get();
    ...
    async getAllMessagesGroupedByUser() {
        ...
        const usersMessages: Record<string, { name: string; phone: string; messages: { message: string; timestamp: admin.firestore.Timestamp }[] }> = {};
